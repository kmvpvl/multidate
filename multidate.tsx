...
class MultiDate extends Component<IMultiDate, IMultiDateExterior> {
    ...
    getDateToView(): DateRange  {
        let cur = this.state.current?this.state.current:strEst.toString();
        let ret = this.getDates().get(cur);
      ...
      default:
        ...
        return (
          <div className={"multidate-container-"+s.style}>
            <div className="multidate-title">{new MLString(this.props.title).toString()}
              <span className="multidate-expandinfo">↕</span>
            </div>
            <div className="multidate-dateselected">{this.getDateTimeString()}<span className="multidate-tolerance">{this.getToleranceString()}</span></div>
            <div className="multidate-dateselector">
                <span className="multidate-dateselector-prev" onClick={()=>this.switchDate(prevInd)}>{prevInd}</span>
                <span className="multidate-dateselector-cur">{cur}</span>
                <span className="multidate-dateselector-next" onClick={()=>this.switchDate(nextInd)}>{nextInd}</span>
            </div>
          </div>
        );
      }
